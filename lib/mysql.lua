...
local function cstring(data) --convert null-term non-empty mysqlclient strings to lua strings
...
local function myerror(mysql)
	local err = cstring(mysqlclient.mysql_error(mysql))
...
function thread_safe()
	return mysqlclient.mysql_thread_safe() == 1
end

function client_info()
	return cstring(mysqlclient.mysql_get_client_info())
end

client_version = mysqlclient.mysql_get_client_version
...
local function proto_ptr(proto) --proto is 'MYSQL_PROTOCOL_*' or mysql.mysqlclient.MYSQL_PROTOCOL_*
...
function connect(t, ...)
	...
	elseif flags then
		for k,v in pairs(flags) do
			local flag = enum(k, 'MYSQL_') --'CLIENT_*' or mysql.mysqlclient.MYSQL_CLIENT_* enum
			client_flag = v and bit.bor(client_flag, flag) or bit.band(client_flag, bit.bnot(flag))
		end
	end

	local mysql = assert(mysqlclient.mysql_init(nil))
	ffi.gc(mysql, mysqlclient.mysql_close)

	if options then
		for k,v in pairs(options) do
			local opt = enum(k) --'MYSQL_OPT_*' or mysql.mysqlclient.MYSQL_OPT_* enum
			local encoder = option_encoders[k]
			if encoder then v = encoder(v) end
			assert(mysqlclient.mysql_options(mysql, opt, ffi.cast('const void*', v)) == 0, 'invalid option')
		end
	end

	if attrs then
		for k,v in pairs(attrs) do
			assert(mysqlclient.mysql_options4(mysql, mysqlclient.MYSQL_OPT_CONNECT_ATTR_ADD, k, v) == 0)
		end
	end

	if key then
		checkz(mysql, mysqlclient.mysql_ssl_set(mysql, key, cert, ca, capath, cipher))
	end

	checkh(mysql, mysqlclient.mysql_real_connect(mysql, host, user, pass, db, port, unix_socket, client_flag))
...
function conn.close(mysql)
	mysqlclient.mysql_close(mysql)
	ffi.gc(mysql, nil)
end

function conn.set_charset(mysql, charset)
	checkz(mysql, mysqlclient.mysql_set_character_set(mysql, charset))
end

function conn.select_db(mysql, db)
	checkz(mysql, mysqlclient.mysql_select_db(mysql, db))
end

function conn.change_user(mysql, user, pass, db)
	checkz(mysql, mysqlclient.mysql_change_user(mysql, user, pass, db))
end

function conn.set_multiple_statements(mysql, yes)
	checkz(mysql, mysqlclient.mysql_set_server_option(mysql, yes and mysqlclient.MYSQL_OPTION_MULTI_STATEMENTS_ON or
																			 mysqlclient.MYSQL_OPTION_MULTI_STATEMENTS_OFF))
end

--connection info

function conn.charset(mysql)
	return cstring(mysqlclient.mysql_character_set_name(mysql))
end

function conn.charset_info(mysql)
	local info = ffi.new'MY_CHARSET_INFO'
	checkz(mysqlclient.mysql_get_character_set_info(mysql, info))
...
function conn.ping(mysql)
	local ret = mysqlclient.mysql_ping(mysql)
	if ret == 0 then
		return true
	elseif mysqlclient.mysql_error(mysql) == mysqlclient.MYSQL_CR_SERVER_GONE_ERROR then
		return false
	end
	myerror(mysql)
end

conn.thread_id = mysqlclient.mysql_thread_id

function conn.stat(mysql)
	return cstring(checkh(mysql, mysqlclient.mysql_stat(mysql)))
end

function conn.server_info(mysql)
	return cstring(checkh(mysql, mysqlclient.mysql_get_server_info(mysql)))
end

function conn.host_info(mysql)
	return cstring(checkh(mysql, mysqlclient.mysql_get_host_info(mysql)))
end

conn.server_version = mysqlclient.mysql_get_server_version
conn.proto_info = mysqlclient.mysql_get_proto_info


function conn.ssl_cipher(mysql)
	return cstring(mysqlclient.mysql_get_ssl_cipher(mysql))
end

--transactions

function conn.commit(mysql) checkz(mysql, mysqlclient.mysql_commit(mysql)) end
function conn.rollback(mysql) checkz(mysql, mysqlclient.mysql_rollback(mysql)) end
function conn.set_autocommit(mysql, yes) checkz(mysql, mysqlclient.mysql_autocommit(mysql, yes == nil or yes)) end
...
function conn.escape_tobuffer(mysql, data, size, buf, sz)
	size = size or #data
	assert(sz >= size * 2 + 1)
	return mysqlclient.mysql_real_escape_string(mysql, buf, data, size)
end
...
function conn.query(mysql, data, size)
	checkz(mysql, mysqlclient.mysql_real_query(mysql, data, size or #data))
end

--query info

conn.field_count = mysqlclient.mysql_field_count

local minus1_uint64 = ffi.cast('uint64_t', ffi.cast('int64_t', -1))
function conn.affected_rows(mysql)
	local n = mysqlclient.mysql_affected_rows(mysql)
	if n == minus1_uint64 then myerror(mysql) end
	return tonumber(n)
end

conn.insert_id = mysqlclient.mysql_insert_id

function conn.errno(conn)
	local err = mysqlclient.mysql_errno(conn)
	if err == 0 then return end
	return err
end

function conn.sqlstate(mysql)
	return cstring(mysqlclient.mysql_sqlstate(mysql))
end

conn.warning_count = mysqlclient.mysql_warning_count

function conn.info(mysql)
	return cstring(mysqlclient.mysql_info(mysql))
end

--query results

function conn.next_result(mysql) --multiple statement queries return multiple results
	local ret = mysqlclient.mysql_next_result(mysql)
...
function conn.more_results(mysql)
	return mysqlclient.mysql_more_results(mysql) == 1
end

local function result_function(func)
	return function(mysql)
		local res = checkh(mysql, func(mysql))
		return ffi.gc(res, mysqlclient.mysql_free_result)
	end
end

conn.store_result = result_function(mysqlclient.mysql_store_result)
conn.use_result = result_function(mysqlclient.mysql_use_result)
...
function res.row_count(res)
	return tonumber(mysqlclient.mysql_num_rows(res))
end

res.field_count = mysqlclient.mysql_num_fields

function res.eof(res)
	return mysqlclient.mysql_eof(res) ~= 0
end

--field info

local field_type_names = {
	[mysqlclient.MYSQL_TYPE_DECIMAL]     = 'decimal',    --DECIMAL or NUMERIC
	[mysqlclient.MYSQL_TYPE_TINY]        = 'tinyint',
	[mysqlclient.MYSQL_TYPE_SHORT]       = 'smallint',
	[mysqlclient.MYSQL_TYPE_LONG]        = 'int',
	[mysqlclient.MYSQL_TYPE_FLOAT]       = 'float',
	[mysqlclient.MYSQL_TYPE_DOUBLE]      = 'double',     --DOUBLE or REAL
	[mysqlclient.MYSQL_TYPE_NULL]        = 'null',
	[mysqlclient.MYSQL_TYPE_TIMESTAMP]   = 'timestamp',
	[mysqlclient.MYSQL_TYPE_LONGLONG]    = 'bigint',
	[mysqlclient.MYSQL_TYPE_INT24]       = 'mediumint',
	[mysqlclient.MYSQL_TYPE_DATE]        = 'date',       --pre mysql 5.0, storage = 4 bytes
	[mysqlclient.MYSQL_TYPE_TIME]        = 'time',
	[mysqlclient.MYSQL_TYPE_DATETIME]    = 'datetime',
	[mysqlclient.MYSQL_TYPE_YEAR]        = 'year',
	[mysqlclient.MYSQL_TYPE_NEWDATE]     = 'date',       --mysql 5.0+, storage = 3 bytes
	[mysqlclient.MYSQL_TYPE_VARCHAR]     = 'varchar',
	[mysqlclient.MYSQL_TYPE_BIT]         = 'bit',
	[mysqlclient.MYSQL_TYPE_TIMESTAMP2]  = 'timestamp',  --mysql 5.6+, can store fractional seconds
	[mysqlclient.MYSQL_TYPE_DATETIME2]   = 'datetime',   --mysql 5.6+, can store fractional seconds
	[mysqlclient.MYSQL_TYPE_TIME2]       = 'time',       --mysql 5.6+, can store fractional seconds
	[mysqlclient.MYSQL_TYPE_NEWDECIMAL]  = 'decimal',    --mysql 5.0+, Precision math DECIMAL or NUMERIC
	[mysqlclient.MYSQL_TYPE_ENUM]        = 'enum',
	[mysqlclient.MYSQL_TYPE_SET]         = 'set',
	[mysqlclient.MYSQL_TYPE_TINY_BLOB]   = 'tinyblob',
	[mysqlclient.MYSQL_TYPE_MEDIUM_BLOB] = 'mediumblob',
	[mysqlclient.MYSQL_TYPE_LONG_BLOB]   = 'longblob',
	[mysqlclient.MYSQL_TYPE_BLOB]        = 'text',       --TEXT or BLOB
	[mysqlclient.MYSQL_TYPE_VAR_STRING]  = 'varchar',    --VARCHAR or VARBINARY
	[mysqlclient.MYSQL_TYPE_STRING]      = 'char',       --CHAR or BINARY
	[mysqlclient.MYSQL_TYPE_GEOMETRY]    = 'spatial',    --Spatial field
}

local binary_field_type_names = {
	[mysqlclient.MYSQL_TYPE_BLOB]        = 'blob',
	[mysqlclient.MYSQL_TYPE_VAR_STRING]  = 'varbinary',
	[mysqlclient.MYSQL_TYPE_STRING]      = 'binary',
}

local field_flag_names = {
	[mysqlclient.MYSQL_NOT_NULL_FLAG]         = 'not_null',
	[mysqlclient.MYSQL_PRI_KEY_FLAG]          = 'pri_key',
	[mysqlclient.MYSQL_UNIQUE_KEY_FLAG]       = 'unique_key',
	[mysqlclient.MYSQL_MULTIPLE_KEY_FLAG]     = 'key',
	[mysqlclient.MYSQL_BLOB_FLAG]             = 'is_blob',
	[mysqlclient.MYSQL_UNSIGNED_FLAG]         = 'unsigned',
	[mysqlclient.MYSQL_ZEROFILL_FLAG]         = 'zerofill',
	[mysqlclient.MYSQL_BINARY_FLAG]           = 'is_binary',
	[mysqlclient.MYSQL_ENUM_FLAG]             = 'is_enum',
	[mysqlclient.MYSQL_AUTO_INCREMENT_FLAG]   = 'autoincrement',
	[mysqlclient.MYSQL_TIMESTAMP_FLAG]        = 'is_timestamp',
	[mysqlclient.MYSQL_SET_FLAG]              = 'is_set',
	[mysqlclient.MYSQL_NO_DEFAULT_VALUE_FLAG] = 'no_default',
	[mysqlclient.MYSQL_ON_UPDATE_NOW_FLAG]    = 'on_update_now',
	[mysqlclient.MYSQL_NUM_FLAG]              = 'is_number',
}
...
function res.field_type(res, i)
	...
	local info = mysqlclient.mysql_fetch_field_direct(res, i-1)
	local unsigned = bit.bor(info.flags, mysqlclient.MYSQL_UNSIGNED_FLAG) ~= 0
...
local function parse_int64(data, sz)
	return ffi.mysqlclient.strtoll(data, nil, 10)
end
...
local field_decoders = { --other field types not present here are returned as strings, unparsed
	[mysqlclient.MYSQL_TYPE_TINY] = parse_int,
	[mysqlclient.MYSQL_TYPE_SHORT] = parse_int,
	[mysqlclient.MYSQL_TYPE_LONG] = parse_int,
	[mysqlclient.MYSQL_TYPE_FLOAT] = parse_float,
	[mysqlclient.MYSQL_TYPE_DOUBLE] = parse_double,
	[mysqlclient.MYSQL_TYPE_TIMESTAMP] = parse_datetime,
	[mysqlclient.MYSQL_TYPE_LONGLONG] = parse_int64,
	[mysqlclient.MYSQL_TYPE_INT24] = parse_int,
	[mysqlclient.MYSQL_TYPE_DATE] = parse_date,
	[mysqlclient.MYSQL_TYPE_TIME] = parse_time,
	[mysqlclient.MYSQL_TYPE_DATETIME] = parse_datetime,
	[mysqlclient.MYSQL_TYPE_NEWDATE] = parse_date,
	[mysqlclient.MYSQL_TYPE_TIMESTAMP2] = parse_datetime,
	[mysqlclient.MYSQL_TYPE_DATETIME2] = parse_datetime,
	[mysqlclient.MYSQL_TYPE_TIME2] = parse_time,
	[mysqlclient.MYSQL_TYPE_YEAR] = parse_int,
	[mysqlclient.MYSQL_TYPE_BIT] = parse_bit,
}

local unsigned_decoders = {
	[mysqlclient.MYSQL_TYPE_LONGLONG] = parse_uint64,
}
...
local function fetch_row(res, numeric, assoc, decode, field_count, fields, t)
	local values = mysqlclient.mysql_fetch_row(res)
	if values == NULL then
	...
	local sizes = mysqlclient.mysql_fetch_lengths(res)
	for i=0,field_count-1 do
		...
		if v ~= NULL then
			...
			if decode then
				...
				local unsigned = bit.bor(fields[i].flags, mysqlclient.MYSQL_UNSIGNED_FLAG) ~= 0
...
function res.fetch(res, mode, t)
	...
	local field_count = mysqlclient.mysql_num_fields(res)
	local fields = fetch_fields and mysqlclient.mysql_fetch_fields(res)
...
res.tell = mysqlclient.mysql_row_tell

function res.seek(res, where) --use in conjunction with res:row_count()
	if type(where) == 'number' then
		mysqlclient.mysql_data_seek(res, where-1)
	else
		mysqlclient.mysql_row_seek(res, where)
	end
end
...
conn.list_dbs = list_function(mysqlclient.mysql_list_dbs)
conn.list_tables = list_function(mysqlclient.mysql_list_tables)
conn.list_processes = result_function(mysqlclient.mysql_list_processes)

--remote control

function conn.kill(mysql, pid)
	checkz(mysql, mysqlclient.mysql_kill(mysql, pid))
end

function conn.shutdown(mysql, level)
	checkz(mysql, mysqlclient.mysql_shutdown(mysql, enum(level or mysqlclient.MYSQL_SHUTDOWN_DEFAULT, 'MYSQL_')))
end

function conn.refresh(mysql, t) --options are 'REFRESH_*' or mysql.mysqlclient.MYSQL_REFRESH_* enums
	local options = 0
	...
	checkz(mysql, mysqlclient.mysql_refresh(mysql, options))
end

function conn.dump_debug_info(mysql)
	checkz(mysql, mysqlclient.mysql_dump_debug_info(mysql))
end

--prepared statements

local function sterror(stmt)
	local err = cstring(mysqlclient.mysql_stmt_error(stmt))
	if not err then return end
	error(string.format('mysql error: %s', err))
end
...
function conn.prepare(mysql, query)
	local stmt = checkh(mysql, mysqlclient.mysql_stmt_init(mysql))
	ffi.gc(stmt, mysqlclient.mysql_stmt_close)
	stcheckz(stmt, mysqlclient.mysql_stmt_prepare(stmt, query, #query))
	return stmt
end
...
function stmt.close(stmt)
	stcheckbool(stmt, mysqlclient.mysql_stmt_close(stmt))
	ffi.gc(stmt, nil)
end

function stmt.exec(stmt)
	stcheckz(stmt, mysqlclient.mysql_stmt_execute(stmt))
end

function stmt.next_result(stmt)
	local ret = mysqlclient.mysql_stmt_next_result(stmt)
	if ret == 0 then return true end
	if ret == -1 then return false end
	sterror(stmt)
end
...
function stmt.store_result(stmt)
	stcheckz(stmt, mysqlclient.mysql_stmt_store_result(stmt))
end

function stmt.free_result(stmt)
	stcheckbool(stmt, mysqlclient.mysql_stmt_free_result(stmt))
end

function stmt.row_count(stmt)
	return tonumber(mysqlclient.mysql_stmt_num_rows(stmt))
end

function stmt.affected_rows(stmt)
	local n = mysqlclient.mysql_stmt_affected_rows(stmt)
	if n == minus1_uint64 then sterror(stmt) end
	return tonumber(n)
end

stmt.insert_id = mysqlclient.mysql_stmt_insert_id
stmt.field_count = mysqlclient.mysql_stmt_field_count
stmt.param_count = mysqlclient.mysql_stmt_param_count

function stmt.errno(stmt)
	local err = mysqlclient.mysql_stmt_errno(stmt)
	if err == 0 then return end
	return err
end

function stmt.sqlstate(stmt)
	return cstring(mysqlclient.mysql_stmt_sqlstate(stmt))
end

function stmt.result_metadata(stmt)
	local res = stcheckh(stmt, mysqlclient.mysql_stmt_result_metadata(stmt))
	return ffi.gc(res, mysqlclient.mysql_free_result)
end
...
function stmt.fetch(stmt)
	local ret = mysqlclient.mysql_stmt_fetch(stmt)
	if ret == 0 then return true end
	if ret == mysqlclient.MYSQL_NO_DATA then return false end
	if ret == mysqlclient.MYSQL_DATA_TRUNCATED then return true, 'truncated' end
	sterror(stmt)
end

function stmt.reset(stmt)
	stcheckz(stmt, mysqlclient.mysql_stmt_reset(stmt))
end

stmt.tell = mysqlclient.mysql_stmt_row_tell

function stmt.seek(stmt, where) --use in conjunction with stmt:row_count()
	if type(where) == 'number' then
		mysqlclient.mysql_stmt_data_seek(stmt, where-1)
	else
		mysqlclient.mysql_stmt_row_seek(stmt, where)
	end
end

function stmt.write(stmt, param_number, data, size)
	stcheckz(stmt, mysqlclient.mysql_stmt_send_long_data(stmt, param_number, data, size or #data))
end

function stmt.update_max_length(stmt)
	local attr = ffi.new'my_bool[1]'
	stcheckz(stmt, mysqlclient.mysql_stmt_attr_get(stmt, mysqlclient.STMT_ATTR_UPDATE_MAX_LENGTH, attr))
	return attr[0] == 1
end

function stmt.set_update_max_length(stmt, yes)
	local attr = ffi.new('my_bool[1]', yes == nil or yes)
	stcheckz(stmt, mysqlclient.mysql_stmt_attr_set(stmt, mysqlclient.STMT_ATTR_CURSOR_TYPE, attr))
end

function stmt.cursor_type(stmt)
	local attr = ffi.new'uint32_t[1]'
	stcheckz(stmt, mysqlclient.mysql_stmt_attr_get(stmt, mysqlclient.STMT_ATTR_CURSOR_TYPE, attr))
	return attr[0]
end
...
function stmt.prefetch_rows(stmt)
	local attr = ffi.new'uint32_t[1]'
	stcheckz(stmt, mysqlclient.mysql_stmt_attr_get(stmt, mysqlclient.STMT_ATTR_PREFETCH_ROWS, attr))
	return attr[0]
end

function stmt.set_prefetch_rows(stmt, n)
	local attr = ffi.new('uint32_t[1]', n)
	stcheckz(stmt, mysqlclient.mysql_stmt_attr_set(stmt, mysqlclient.STMT_ATTR_PREFETCH_ROWS, attr))
end
...
local bb_types_input = {
	...
	tinyint    = mysqlclient.MYSQL_TYPE_TINY,
	smallint   = mysqlclient.MYSQL_TYPE_SHORT,
	int        = mysqlclient.MYSQL_TYPE_LONG,
	integer    = mysqlclient.MYSQL_TYPE_LONG,       --alias of int
	bigint     = mysqlclient.MYSQL_TYPE_LONGLONG,
	float      = mysqlclient.MYSQL_TYPE_FLOAT,
	double     = mysqlclient.MYSQL_TYPE_DOUBLE,
	time       = mysqlclient.MYSQL_TYPE_TIME,
	date       = mysqlclient.MYSQL_TYPE_DATE,
	datetime   = mysqlclient.MYSQL_TYPE_DATETIME,
	timestamp  = mysqlclient.MYSQL_TYPE_TIMESTAMP,
	text       = mysqlclient.MYSQL_TYPE_STRING,
	char       = mysqlclient.MYSQL_TYPE_STRING,
	varchar    = mysqlclient.MYSQL_TYPE_STRING,
	blob       = mysqlclient.MYSQL_TYPE_BLOB,
	binary     = mysqlclient.MYSQL_TYPE_BLOB,
	varbinary  = mysqlclient.MYSQL_TYPE_BLOB,
	null       = mysqlclient.MYSQL_TYPE_NULL,
	--conversion types (can only use one of the above mysqlclient types)
	mediumint  = mysqlclient.MYSQL_TYPE_LONG,
	real       = mysqlclient.MYSQL_TYPE_DOUBLE,
	decimal    = mysqlclient.MYSQL_TYPE_BLOB,
	numeric    = mysqlclient.MYSQL_TYPE_BLOB,
	year       = mysqlclient.MYSQL_TYPE_SHORT,
	tinyblob   = mysqlclient.MYSQL_TYPE_BLOB,
	tinytext   = mysqlclient.MYSQL_TYPE_BLOB,
	mediumblob = mysqlclient.MYSQL_TYPE_BLOB,
	mediumtext = mysqlclient.MYSQL_TYPE_BLOB,
	longblob   = mysqlclient.MYSQL_TYPE_BLOB,
	longtext   = mysqlclient.MYSQL_TYPE_BLOB,
	bit        = mysqlclient.MYSQL_TYPE_LONGLONG, --MYSQL_TYPE_BIT is not available for input params
	set        = mysqlclient.MYSQL_TYPE_BLOB,
	enum       = mysqlclient.MYSQL_TYPE_BLOB,
}
...
local bb_types_output = {
	...
	tinyint    = mysqlclient.MYSQL_TYPE_TINY,
	smallint   = mysqlclient.MYSQL_TYPE_SHORT,
	mediumint  = mysqlclient.MYSQL_TYPE_INT24,      --int32
	int        = mysqlclient.MYSQL_TYPE_LONG,
	integer    = mysqlclient.MYSQL_TYPE_LONG,       --alias of int
	bigint     = mysqlclient.MYSQL_TYPE_LONGLONG,
	float      = mysqlclient.MYSQL_TYPE_FLOAT,
	double     = mysqlclient.MYSQL_TYPE_DOUBLE,
	real       = mysqlclient.MYSQL_TYPE_DOUBLE,
	decimal    = mysqlclient.MYSQL_TYPE_NEWDECIMAL, --char[]
	numeric    = mysqlclient.MYSQL_TYPE_NEWDECIMAL, --char[]
	year       = mysqlclient.MYSQL_TYPE_SHORT,
	time       = mysqlclient.MYSQL_TYPE_TIME,
	date       = mysqlclient.MYSQL_TYPE_DATE,
	datetime   = mysqlclient.MYSQL_TYPE_DATETIME,
	timestamp  = mysqlclient.MYSQL_TYPE_TIMESTAMP,
	char       = mysqlclient.MYSQL_TYPE_STRING,
	binary     = mysqlclient.MYSQL_TYPE_STRING,
	varchar    = mysqlclient.MYSQL_TYPE_VAR_STRING,
	varbinary  = mysqlclient.MYSQL_TYPE_VAR_STRING,
	tinyblob   = mysqlclient.MYSQL_TYPE_TINY_BLOB,
	tinytext   = mysqlclient.MYSQL_TYPE_TINY_BLOB,
	blob       = mysqlclient.MYSQL_TYPE_BLOB,
	text       = mysqlclient.MYSQL_TYPE_BLOB,
	mediumblob = mysqlclient.MYSQL_TYPE_MEDIUM_BLOB,
	mediumtext = mysqlclient.MYSQL_TYPE_MEDIUM_BLOB,
	longblob   = mysqlclient.MYSQL_TYPE_LONG_BLOB,
	longtext   = mysqlclient.MYSQL_TYPE_LONG_BLOB,
	bit        = mysqlclient.MYSQL_TYPE_BIT,
	--conversion types (can only use one of the above mysqlclient types)
	null       = mysqlclient.MYSQL_TYPE_TINY,
	set        = mysqlclient.MYSQL_TYPE_BLOB,
	enum       = mysqlclient.MYSQL_TYPE_BLOB,
}

local number_types = {
	[mysqlclient.MYSQL_TYPE_TINY]      = 'int8_t[1]',
	[mysqlclient.MYSQL_TYPE_SHORT]     = 'int16_t[1]',
	[mysqlclient.MYSQL_TYPE_LONG]      = 'int32_t[1]',
	[mysqlclient.MYSQL_TYPE_INT24]     = 'int32_t[1]',
	[mysqlclient.MYSQL_TYPE_LONGLONG]  = 'int64_t[1]',
	[mysqlclient.MYSQL_TYPE_FLOAT]     = 'float[1]',
	[mysqlclient.MYSQL_TYPE_DOUBLE]    = 'double[1]',
}

local uint_types = {
	[mysqlclient.MYSQL_TYPE_TINY]      = 'uint8_t[1]',
	[mysqlclient.MYSQL_TYPE_SHORT]     = 'uint16_t[1]',
	[mysqlclient.MYSQL_TYPE_LONG]      = 'uint32_t[1]',
	[mysqlclient.MYSQL_TYPE_INT24]     = 'uint32_t[1]',
	[mysqlclient.MYSQL_TYPE_LONGLONG]  = 'uint64_t[1]',
}

local time_types = {
	[mysqlclient.MYSQL_TYPE_TIME]      = true,
	[mysqlclient.MYSQL_TYPE_DATE]      = true,
	[mysqlclient.MYSQL_TYPE_DATETIME]  = true,
	[mysqlclient.MYSQL_TYPE_TIMESTAMP] = true,
}

local time_struct_types = {
	[mysqlclient.MYSQL_TYPE_TIME] = mysqlclient.MYSQL_TIMESTAMP_TIME,
	[mysqlclient.MYSQL_TYPE_DATE] = mysqlclient.MYSQL_TIMESTAMP_DATE,
	[mysqlclient.MYSQL_TYPE_DATETIME] = mysqlclient.MYSQL_TIMESTAMP_DATETIME,
	[mysqlclient.MYSQL_TYPE_TIMESTAMP] = mysqlclient.MYSQL_TIMESTAMP_DATETIME,
}
...
local function bind_buffer(bb_types, meta, types)
	...
	for i,typedef in ipairs(types) do
		...
		local data
		if stype == 'bit' then
			if btype == mysqlclient.MYSQL_TYPE_LONGLONG then --for input: use unsigned int64 and ignore size
				data = ffi.new'uint64_t[1]'
				self.buffer[i-1].is_unsigned = 1
				size = 0
			elseif btype == mysqlclient.MYSQL_TYPE_BIT then --for output: use mysql conversion-free type
		...
		elseif time_types[btype] then
			...
			size = 0
		elseif btype == mysqlclient.MYSQL_TYPE_NULL then
			assert(not size, 'fixed size type')
			size = 0
		else
...
function fields:get_date(i)
	...
	local date = btype == mysqlclient.MYSQL_TYPE_DATE or btype == mysqlclient.MYSQL_TYPE_DATETIME or btype == mysqlclient.MYSQL_TYPE_TIMESTAMP
	local time = btype == mysqlclient.MYSQL_TYPE_TIME or btype == mysqlclient.MYSQL_TYPE_DATETIME or btype == mysqlclient.MYSQL_TYPE_TIMESTAMP
...
function params:set(i, v, size)
	...
	local btype = tonumber(self.buffer[i-1].buffer_type)
	if btype == mysqlclient.MYSQL_TYPE_NULL then
...
function fields:get(i)
	bind_check_range(self, i)
	local btype = tonumber(self.buffer[i-1].buffer_type)
	if btype == mysqlclient.MYSQL_TYPE_NULL or self.null_flags[i-1] == 1 then
		return nil
	end
	...
	elseif time_types[btype] then
		local t = self.data[i]
		if t.time_type == mysqlclient.MYSQL_TIMESTAMP_TIME then
			return datetime{hour = t.hour, min = t.minute, sec = t.second, frac = t.second_part}
		elseif t.time_type == mysqlclient.MYSQL_TIMESTAMP_DATE then
			return datetime{year = t.year, month = t.month, day = t.day}
		elseif t.time_type == mysqlclient.MYSQL_TIMESTAMP_DATETIME then
			return datetime{year = t.year, month = t.month, day = t.day,
		...
		if btype == mysqlclient.MYSQL_TYPE_BIT then
...
function fields:is_null(i) --returns true if the field is null
	...
	return btype == mysqlclient.MYSQL_TYPE_NULL or self.null_flags[i-1] == 1
end
...
function stmt.bind_params(stmt, ...)
	...
	stcheckz(stmt, mysqlclient.mysql_stmt_bind_param(stmt, bb.buffer))
	return bb
end
...
function stmt.bind_result(stmt, arg1, ...)
	...
	stcheckz(stmt, mysqlclient.mysql_stmt_bind_result(stmt, bb.buffer))
	return bb
end
